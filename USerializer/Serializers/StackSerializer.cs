...
{
    public sealed unsafe class StackSerializer : CustomDataSerializer
    {
        ...
        public StackSerializer(Type fieldType, Type elementType, DataSerializer elementSerializer)
        {
        ...
        public override void Write(ReadOnlySpan<byte> span, ref SerializerOutput output)
        {
            ...
            var count = _sizeField.GetFieldRef(ref instance);
            ...
            if (count > 0)
            {
                ...
                fixed (byte* address = pinnable)
                {
                    ...
                    // Stack stores elements from Bottom to Top in the array [0..count-1]
                    // We want to write [Bottom, ..., Top] so deserialization (Push) preserves order.
                    for (int i = 0; i < count; i++)
                    {
                        serializer.Write(new ReadOnlySpan<byte>(address + (i * _elementSize), _elementSize),
                            ref output);
                    }
                }
        ...
        public override void Read(Span<byte> span, ref SerializerInput input)
        {
            ...
            if (input.BeginReadSize(out var end))
            {
                ...
                if (instance == null)
                {
                    ...
                    _itemsField.GetFieldRef(ref instance) = Array.CreateInstance(_elementType, count);
                    _sizeField.GetFieldRef(ref instance) = count;
                }
                else
                {
                    ref var arrayRef = ref _itemsField.GetFieldRef(ref instance);
                    ref var currentCountRef = ref _sizeField.GetFieldRef(ref instance);

                    if (arrayRef.Length < count)
                    {
                ...
                var array = _itemsField.GetFieldRef(ref instance);
